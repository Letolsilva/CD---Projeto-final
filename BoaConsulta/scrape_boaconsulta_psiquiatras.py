# scrape_boa_consulta_psiquiatras.py
...
# Exemplo com apenas uma cidade (você pode ampliar para uma lista como CITY_SLUGS)
CITY_SLUGS = [
    "sao-paulo-sp",
    "rio-de-janeiro-rj",
    "belo-horizonte-mg",
...
OUTPUT_FILE = "psiquiatras_boaconsulta.csv"
CACHE_DIR = "cache_profiles_psiquiatras"
...
def get_list_url_for_city(city_slug, page=None):
    # Aqui usamos “psiquiatras” em vez de “psicologia-geral”
    base = f"{BASE}/especialistas/psiquiatras/{city_slug}/particular?s=2"
    if page and page > 1:
...
def fetch_page(url):
    ...
    # fallback: usar html.parser se lxml não estiver disponível
    try:
...
def extract_crp_and_price(profile_url):
    ...
    # CRP ou registro médico (no caso de psiquiatra, pode ser CRM)
    crp = None
    el = soup.select_one("p.text-xs")
    if el:
        text = el.get_text(strip=True)
        # para psiquiatra, pode dizer “CRP” ou “CRM” – vamos aceitar ambos
    ...
    # Cidade do perfil — pega o elemento com classe “speakable-locations-name”
    city = None
...
def parse_cards(soup, city_slug, writer, seen):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_prof):
            ...
            # somente salva se preço existir
            if not price:
                continue
